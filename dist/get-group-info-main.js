...
define(["require", "exports", "@esri/solution-common"], function (require, exports, common) {
    ...
    function getGroupInfo(groupId, authentication) {
        return new Promise((resolve, reject) => {
            ...
            const groupThumbnailDef = new Promise((resolve3, reject3) => {
                // tslint:disable-next-line: no-floating-promises
                groupBaseDef.then(
                // any error fetching item base will be handled via Promise.all later
                (groupBase) => {
                    common
                        .getItemThumbnail(groupId, groupBase.thumbnail, true, authentication)
                        .then(resolve3, (error) => reject3(JSON.stringify(error)));
                });
            });
            ...
            Promise.all([
                ...
                const [groupBase, groupData, groupThumbnail, groupCategorySchema] = responses;
                ...
                if (groupCategorySchema.categorySchema.length === 0) {
                    ...
                    html += textAreaHtml(JSON.stringify(groupCategorySchema.categorySchema, null, 2));
                }
                html += "</div></p>";
                resolve(html);
    ...
    function showBlob(blob) {
        ...
        return new Promise(resolve => {
            ...
            if (blob.type === "application/json") {
                common.blobToJson(blob).then(text => resolve(textAreaHtml(JSON.stringify(text, null, 2)) + addFilename(filename)), error => resolve("<i>problem extracting JSON: " + error + "</i>"));
            }
            else if (blob.type.startsWith("text/plain") ||
                blob.type === "text/xml" ||
                blob.type === "application/xml") {
                common.blobToText(blob).then(text => resolve(textAreaHtml(text) + addFilename(filename)), error => resolve("<i>problem extracting text: " + error + "</i>"));
            }
...
//# sourceMappingURL=get-group-info-main.js.map