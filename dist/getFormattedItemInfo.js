...
define(["require", "exports", "@esri/solution-common"], function (require, exports, common) {
    ...
    exports.getFormattedItemInfo = void 0;
    // ------------------------------------------------------------------------------------------------------------------ //
    function getFormattedItemInfo(itemId, authentication, Raphael, Dracula) {
        return new Promise((resolve, reject) => {
            ...
            common.getCompleteItem(itemId, authentication)
                ...
                await formatItemInfo(portalUrl, item, Raphael, Dracula)
                    .then(html => resolve(html), reject);
            }, reject);
        });
    }
    exports.getFormattedItemInfo = getFormattedItemInfo;
    // ------------------------------------------------------------------------------------------------------------------ //
    function addFilename(filename) {
        return filename ? "&nbsp;" + filename : "";
    }
    function createGraphTag(template) {
        return common.getItemTypeAbbrev(template.type) + ' ' + template.itemId.substr(0, 6);
    }
    async function formatItemInfo(portalUrl, item, Raphael, Dracula) {
    ...
    `;
        ...
        if (item.fwdRelatedItems.length === 0 &&
        ...
        if (item.featureServiceProperties) {
            ...
            html +=
                "<p><i>Service description</i><br/>" +
                    ...
                    "</p>";
            html += "<p><i>Layers</i>";
            item.featureServiceProperties.layers.forEach(layer => (html += textAreaHtmlFromJSON(layer)));
            html += "</p>";
            html += "<p><i>Tables</i>";
            item.featureServiceProperties.tables.forEach(layer => (html += textAreaHtmlFromJSON(layer)));
            html += "</p>";
            html += "</p>";
            return html;
        }
            ...
            setTimeout(() => {
                const topologicalSortGraphicDiv = document.getElementById("topologicalSortGraphic");
    ...
    function showBlob(blob) {
        ...
        return new Promise(resolve => {
            ...
            if (blob.type === "application/json") {
                common.blobToJson(blob).then(text => resolve(textAreaHtmlFromJSON(text) + addFilename(filename)), error => resolve("<i>problem extracting JSON: " + error + "</i>"));
            }
            else if (blob.type.startsWith("text/plain") ||
                blob.type === "text/xml" ||
                blob.type === "application/xml") {
                common.blobToText(blob).then(text => resolve(textAreaHtmlFromText(text) + addFilename(filename)), error => resolve("<i>problem extracting text: " + error + "</i>"));
            }
    ...
    function showTopologicalSortGraph(collection, canvas, width, height, Raphael, Dracula) {
...
//# sourceMappingURL=getFormattedItemInfo.js.map