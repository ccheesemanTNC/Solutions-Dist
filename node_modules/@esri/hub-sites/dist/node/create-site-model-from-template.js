...
exports.createSiteModelFromTemplate = void 0;
const hub_common_1 = require("@esri/hub-common");
const hub_teams_1 = require("@esri/hub-teams");
const get_theme_1 = require("./get-theme");
const get_portal_site_hostname_1 = require("./get-portal-site-hostname");
const get_portal_site_url_1 = require("./get-portal-site-url");
const _create_site_initiative_1 = require("./_create-site-initiative");
const _update_team_tags_1 = require("./_update-team-tags");
const hub_common_2 = require("@esri/hub-common");
const interpolate_site_1 = require("./interpolate-site");
/**
...
function createSiteModelFromTemplate(template, settings, transforms, hubRequestOptions) {
    ...
    let title = hub_common_1.getProp(settings, "solution.title") || "New Site";
    // handle issue with titles that are just numbers
    if (typeof title === "number") {
        title = title.toString();
        hub_common_1.deepSet(settings, "solution.title", title);
    }
    ...
    return hub_teams_1.createHubTeams({
        title,
        types: teamsToCreate,
        hubRequestOptions,
    })
        ...
        settings.teams = hub_common_1.cloneObject(teams.props);
        state.teams = teams;
        ...
        if (hub_common_1.getProp(teams, "props.contentGroupId")) {
            hub_common_1.deepSet(template, "data.catalog.groups", [teams.props.contentGroupId]);
        }
        ...
        return hub_common_2.ensureUniqueDomainName(hub_common_1.slugify(domainTitle), hubRequestOptions);
    })
        ...
        settings.solution.theme = get_theme_1.getTheme(portal);
        // set site-specific settings properties...
        settings.solution.subdomain = uniqueSubdomain;
        // setup the url properties
        if (hubRequestOptions.isPortal) {
            settings.solution.defaultHostname = get_portal_site_hostname_1.getPortalSiteHostname(uniqueSubdomain, portal);
            settings.solution.url = get_portal_site_url_1.getPortalSiteUrl(uniqueSubdomain, portal);
        }
        else {
            const base = hub_common_1.stripProtocol(hub_common_1.getHubApiUrl(hubRequestOptions));
        ...
        if (product !== "portal") {
            handleInitiative = _create_site_initiative_1._createSiteInitiative(template, settings, transforms, hubRequestOptions);
        }
        ...
        if (maybeInitiative) {
            ...
            teamUpdatePromise = _update_team_tags_1._updateTeamTags(maybeInitiative, state.teams, hubRequestOptions);
        }
        return teamUpdatePromise;
    })
        .then((_) => {
        const siteModel = interpolate_site_1.interpolateSite(template, settings, transforms);
        ...
        if (typeof siteModel.item.title === "number") {
            siteModel.item.title = hub_common_1.getProp(settings, "solution.title");
            siteModel.data.values.title = hub_common_1.getProp(settings, "solution.title");
        }
        return siteModel;
    })
        .catch((ex) => {
...
//# sourceMappingURL=create-site-model-from-template.js.map